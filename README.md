...
| `GFMT_FIRMWARE_BUILD_TTL_S` | 2592000 (30 days) | Delete a firmware build sandbox once it's sat around this long (seconds), even if under the keep-count. 0 disables the age sweep, leaving only the count cap above. |
| `GFMT_ESP_IDF_IDLE_TTL_S` | 2592000 (30 days) | Delete the on-demand ESP-IDF toolchain after this much idle time (seconds). The next build just re-downloads it. 0 keeps the toolchain indefinitely once provisioned. |
...
- Everything you'd want to back up - config, credentials, logs - lives in the one data directory you control; the on-demand ESP-IDF toolchain and build files (`GFMT_FIRMWARE_DIR`) are entirely rebuildable and intentionally kept outside that mount. Nothing phones home except to Google's own APIs and (if you configure it) your Apprise notification targets.
